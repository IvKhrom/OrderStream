...
// Provider отдаёт Swagger/OpenAPI JSON (через интерфейс).
type Provider interface {
	SwaggerJSON(ctx context.Context) ([]byte, error)
}

// Router — минимальный интерфейс роутера (подходит chi.Router).
...
func (EmbeddedProvider) SwaggerJSON(_ context.Context) ([]byte, error) {
	...
	if err != nil {
		return nil, errors.New("swagger file not found: generate it for api_service")
	}
	return b, nil
}
