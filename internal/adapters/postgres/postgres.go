...
func (p *Postgres) CreateOrder(ctx context.Context, o *domain.Order) error {
	...
	sql := `INSERT INTO orders(order_id, external_id, user_id, amount, status, payload, bucket)
            VALUES ($1,$2,$3,$4,$5,$6,$7)`

	_, err := p.pool.Exec(ctx, sql, o.OrderID, o.ExternalID, o.UserID, o.Amount, o.Status, payload, o.Bucket)
	return err
}

func (p *Postgres) GetByID(ctx context.Context, id uuid.UUID) (*domain.Order, error) {
	row := p.pool.QueryRow(ctx,
		`SELECT order_id, external_id, user_id, amount, status, payload, created_at, updated_at, bucket 
         FROM orders WHERE order_id=$1`, id)

	var o domain.Order
	var payload []byte
	if err := row.Scan(&o.OrderID, &o.ExternalID, &o.UserID, &o.Amount, &o.Status, &payload, &o.CreatedAt, &o.UpdatedAt, &o.Bucket); err != nil {
		return nil, err
	}
...
func (p *Postgres) GetByExternalID(ctx context.Context, externalID string, userID uuid.UUID) (*domain.Order, error) {
	row := p.pool.QueryRow(ctx,
		`SELECT order_id, external_id, user_id, amount, status, payload, created_at, updated_at, bucket 
         FROM orders WHERE external_id=$1 AND user_id=$2 LIMIT 1`,
		externalID, userID)
